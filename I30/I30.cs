...
public class I30
{
    public I30(Stream fileStream)
    {
        ...
        {
            while (br.BaseStream.Position < br.BaseStream.Length) pages.Add(br.ReadBytes(pageSize));
        }
        ...
        foreach (var page in pages)
        {
            ...
            Log.Debug("Procesing page 0x{PageNumber:X}", pageNumber);

            using (var br = new BinaryReader(new MemoryStream(page)))
            {
                var sigActual = br.ReadInt32();

                if (sig != sigActual) throw new Exception("Invalid header! Expected 'INDX' Signature.");
                ...
                while (br.BaseStream.Position % 8 != 0) br.ReadByte(); //gets us past padding
                ...
                using (var binaryReader = new BinaryReader(new MemoryStream(slackSpace)))
                {
                    ...
                    {
                        var absoluteOffset = pageNumber * 0x1000 + 0x18 + dataStartOffset +
                                             binaryReader.BaseStream.Position + activeSpace.Length;

                        Log.Information(
                            "IN SLACK LOOP: Absolute offset: 0x{AbsoluteOffset:X} brActive.BaseStream.Position: 0x{Position:X}",
                            absoluteOffset, binaryReader.BaseStream.Position);
...
public class IndexEntry
{
    ...
    public IndexEntry(byte[] rawBytes, long absoluteOffset, int pageNumber, bool fromSlack)
    {
        ...
        using (var br = new BinaryReader(new MemoryStream(rawBytes)))
        {
            ...
            if (MftReferenceSelf.MftEntryNumber == 0) return;
